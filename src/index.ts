...
async function executeTypeScriptCode(code: string): Promise<{ output: string; error?: string }> {
...
async function executeJavaScriptCode(code: string): Promise<{ output: string; error?: string }> {
...
server.tool(
  "validate-typescript",
  "TypeScriptコードの型チェックを行います（実行はしません）",
  {
